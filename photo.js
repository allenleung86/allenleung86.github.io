...
qiniu.conf.ACCESS_KEY = 'Su5UOz3WLyZ6E-Vt3qzuMamkbXHMv5p0256UYe9C';
qiniu.conf.SECRET_KEY = 'e8PFrFFrdLf_RtBhHwKPTPxYG7FoE2u_sCEb7C-O';
...
fs.readdir(path, function (err, files) {
  ...
  (function iterator(index) {
    if (index == files.length) {
      fs.writeFile("./source/photos/photoData.json", JSON.stringify(arr, null, "\t"));
      return;
    }
    fs.stat(path + files[index], function (err, stats) {
      ...
      if (stats.isFile()) {
        var imgExt = new Array(".png", ".jpg", ".jpeg", ".bmp", ".gif", ".ico"); //图片文件的后缀名
